...
impl DiffProviderRegistry {
    ...
    ///
    /// Each registered provider reports its own, so there is no workspace to scan: a
    /// provider is registered per repository root and already holds an open repo.
    pub fn get_watched_paths(&self) -> Vec<PathBuf> {
        self.providers
            .values()
            .filter_map(|provider| provider.get_watched_path())
            .collect()
    }
...
impl DiffProvider {
    ...
    ///
    /// No workspace argument any more: providers are now registered per repository root
    /// with an already-open repo, so each one reports its own HEAD rather than
    /// rediscovering a repository from the workspace path.
    fn get_watched_path(&self) -> Option<PathBuf> {
        match *self {
            #[cfg(feature = "git")]
...
/// Should a change to this path invalidate the git diff base?
///
/// Split out of `DiffProvider::needs_reload` to keep it testable. The check is purely
/// path-based, but constructing a `DiffProvider::Git` now requires an open repository,
/// so the test would otherwise need a real repo on disk to exercise string matching.
#[cfg(feature = "git")]
...
mod tests {
    ...
    fn test_needs_reload_regular_git() {
        ...
        assert!(is_git_head_path(Path::new("/home/user/repo/.git/HEAD")));

        // Other .git files should not trigger reload
        assert!(!is_git_head_path(Path::new(
