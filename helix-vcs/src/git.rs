...
/// Get the path to the HEAD file of an already-opened repository.
///
/// Takes the repo rather than a path because a `DiffProvider::Git` now carries the
/// repository it opened at registration, so there is nothing to rediscover here.
/// git_dir() handles both layouts: /path/to/repo/.git for a regular repo, and
/// /path/to/main/.git/worktrees/<name> for a worktree.
pub(super) fn get_head_path(repo: &ThreadSafeRepository) -> Option<PathBuf> {
    let repo = repo.to_thread_local();
